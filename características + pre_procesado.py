...
def procesar_archivos(directorio='datos', salida='dataset', tiempo=5, fs=500, solape=0.95):
    ...
    for archivo in archivos:
        try:
            ...
            if 'data' not in datos or 'temblor' not in datos:
                print(f"Saltando {archivo}: faltan variables 'data' o 'temblor'")
                continue

            emg = datos['data'].squeeze()
            temblor = datos['temblor'].squeeze()
            gen = datos['genero']
            edad= datos['edad']
            
            print("print del emg",emg)
...
procesar_archivos(directorio='emg/com_ext', tiempo=3)
